...
const isBL = (user: TeilnehmendenverwalterIn): boolean => {
  return user.tribe === "1312" && user.level !== "all" && user.level !== "kitchen"
}

const isLeader = (user: TeilnehmendenverwalterIn): boolean => {
  return user.tribe !== "1312" && user.level !== "all" && user.level !== "kitchen"
}

const isCreationAllowed = (req: any): boolean => {
  ...
  if (isStavo(controller)) {
    return tribe === Number(controller.tribe) && level !== "all" && level !== "kitchen"
  }
  return false
}
