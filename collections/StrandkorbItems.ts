...
import { isBevo, isStrandkorb, ParticipantsControllerUser } from './ParticipantsController';
import { TeilnehmendenverwalterIn } from '../payload-types';
import { AdminAccess } from './Participants';

const StrandkorbAccess: Access = ({ req: { user } }: { req: { user: ParticipantsControllerUser }}): boolean => {
  return user && (user.collection === "users" || isBevo(user as TeilnehmendenverwalterIn) || isStrandkorb(user as TeilnehmendenverwalterIn))
}
