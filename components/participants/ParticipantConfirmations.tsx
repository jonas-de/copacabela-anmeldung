...
import {PhotoPermissionText, StateText} from '../../utilitites/Wording';
...
const ParticipantConfirmations: React.FC<{
  ...
  const [form] = useForm();
  const [photoPermission, setPhotoPermission] = useState<PhotoPermissionText>(
    participant.receivedPhotoPermission
  );
  ...
  const onSubmit = async (values: ConfirmationParticipant) => {
  ...
  const confirm = async () => {
    const values: ConfirmationParticipant = {
      receivedRegistration: form.getFieldValue(
        'receivedRegistration'
      ) as boolean,
      receivedLeaderInfo: form.getFieldValue('receivedLeaderInfo') as boolean,
      state: 'confirmed',
    };
  ...
  const isComplete = (): boolean => {
    if (
      ...
      !form.getFieldValue('receivedLeaderInfo')
    ) {
      return false;
    }
    return !(
      !form.getFieldValue('receivedRegistration') || photoPermission === 'no'
    );
  };
  ...
  return (
    ...
    >
      ...
      <Form.Item name="photopermission" valuePropName="checked">
        <Checkbox
          onChange={e => updatePhotoPermission(e.target.checked)}
          disabled={photoPermission === 'never'}
        >
          Fotoerlaubnis abgegeben
        </Checkbox>
      </Form.Item>
      <Form.Item name="noPhotopermission" valuePropName="checked">
        <Checkbox onChange={e => updateNoPhotoPermission(e.target.checked)}>
          Gibt keine Fotoerlaubnis ab
        </Checkbox>
      </Form.Item>
      <Form.Item>
        <Row gutter={8}>
