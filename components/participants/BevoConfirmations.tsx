...
const BevoConfirmations: React.FC<{participant: TeilnehmerIn}> = ({
  participant,
}) => {
  const boolRules: Rule[] = [{type: 'boolean', required: true}];
  const defaultWidth: {width: number} = {width: 240};

  const onSubmit = async (values: {review: {at: Date}}) => {
    values.review.at = (values.review.at as unknown as moment.Moment).toDate();
  ...
  return (
    ...
    >
      ...
      >
        <Button type="primary" htmlType="submit" style={defaultWidth}>
          Einsicht speichern
        </Button>
      </Form.Item>
    </Form>
