...
import { ExceptionOutlined, FileImageOutlined, FileOutlined } from '@ant-design/icons';

const DocumentView: React.FC<{ tn: TeilnehmerIn, isBevo: boolean }> = ({ tn, isBevo }) => {
  ...
  return (<>
    <Checkbox disabled={!isBevo} checked={anmeldung} onClick={updateAnmeldung}><FileOutlined /></Checkbox>
    <Checkbox disabled={!isBevo || photo === "never"} checked={photo === "yes"} onClick={updatePhoto}><FileImageOutlined /></Checkbox>
    { tn.role !== "participant" && (
      <Checkbox disabled={!isBevo} checked={leader} onClick={updateLeader}><ExceptionOutlined /></Checkbox>
    )}
  </>)
}
