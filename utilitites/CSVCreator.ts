...
const Fields = {
  ...
  tribe: {
    ...
    title: "Stamm",
    head: "Stamm",
    toText: (tn: TeilnehmerIn) => getTribeForNumber(Number(tn.tribe)).name,
  },
  role: {
    slug: "role",
    title: "Rolle",
  ...
  cancelDate: {
    ...
    toText: (tn: TeilnehmerIn) => {
      if (tn.cancelledAt !== undefined) {
        return moment(tn.cancelledAt).tz("Europe/Berlin").format("DD.MM.YYYY")
      } else {
        return ""
      }
    },
  ...
  birthDate: {
    ...
    title: "Geburtstag",
    head: "Geburtstag",
    toText: (tn: TeilnehmerIn) => moment(tn.birthDate).tz("Europe/Berlin").format("DD.MM.YYYY"),
  },
  gender: {
    slug: "gender",
    title: "Geschlecht",
  ...
  eatingBehaviour: {
    ...
    title: "Essverhalten",
    head: "Essverhalten",
    toText: (tn: TeilnehmerIn) => getEatingBehaviour(tn.food.eatingBehaviour).name,
  },
  intolerances: {
    slug: "intolerances",
  ...
  tetanus: {
    ...
    toText: (tn: TeilnehmerIn) => tn.vaccinations.tetanus,
  },
  covid: {
    slug: "covid",
    title: "Corona-Impfung",
    head: "Corona-Impfung",
    toText: (tn: TeilnehmerIn) => getCovidVaccinationState(tn.vaccinations.covid).name,
  },
  fsme: {
    slug: "fsme",
    title: "FSME-Impfung",
    head: "FSME-Impfung",
    toText: (tn: TeilnehmerIn) => tn.vaccinations.fsme,
  },
  diseases: {
    slug: "diseases",
    title: "Krankheiten",
  ...
  healthInsurance: {
    ...
    toText: (tn: TeilnehmerIn) => getInsuranceType(tn.healthInsurance).name,
  },
  swimmer: {
    slug: "swimmer",
    title: "Schwimmer:in",
  ...
  firstContact: {
    ...
    head: "Notfallkontakt;Notfallnummer",
    toText: (tn: TeilnehmerIn) => {
      if (tn.legalGuardian !== undefined) {
        return `${tn.legalGuardian.name || ""}: ${tn.legalGuardian.phoneNumber || ""}`
      }
      if (tn.contacts !== undefined) {
        return `${tn.contacts[0].name}: ${tn.contacts[0].phoneNumber}`
      }
    },
  },
  presencePerDay: {
    ...
    toText: (tn: TeilnehmerIn) => {
      return Object.values(tn.presence).map((present) => present ? "Ja" : "Nein").join(";")
    },
  },
  presence: {
    slug: "presence",
  ...
  juleica: {
    ...
    toText: (tn: TeilnehmerIn) => {
      ...
      if (tn.juleica?.terminates !== undefined) {
        info.push(moment(tn.juleica.terminates).tz("Europe/Berlin").format("DD.MM.YYYY"))
      } else {
        info.push("")
      }
      return info.join(";")
  ...
  course2d2e: {
    ...
    toText: (tn: TeilnehmerIn) => {
      if (tn.course !== undefined) {
        return moment(tn.course).tz("Europe/Berlin").format("DD.MM.YYYY")
      } else {
        return ""
      }
    },
  ...
  photoPermission: {
    ...
    title: "Fotoerlaubnis",
    head: "Fotoerlaubnis",
    toText: (tn: TeilnehmerIn) => getPhotopermissionState(tn.receivedPhotoPermission).name,
  },
  leaderInfo: {
    slug: "leaderInfo",
