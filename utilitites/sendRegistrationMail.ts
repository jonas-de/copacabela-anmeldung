...
const generatePhotoPermission = async (
  name: string,
  birthDate: Moment
): Promise<Uint8Array | null> => {
  try {
    const file = await fs.promises.readFile(
      `documents/photopermission/Fotoerlaubnis-CopacaBeLa-${
        hasLegalAge(birthDate) ? 'ab18' : 'u18'
      }.pdf`
    );
...
const generateRegistration = async (
  particpant: TeilnehmerIn
): Promise<Uint8Array | null> => {
  try {
    const tn = particpant as TeilnehmerIn & {createdAt: string};
    const createdAt = tn.createdAt
      ? moment(tn.createdAt).tz('Europe/Berlin').format('DD.MM.YYYY')
      : '';
    const data = `${createdAt}\t${particpant.orderId}`;

    const {street, zipCode, city} = particpant.address;
    const addressText = `${street}, ${zipCode} ${city}`;

    const levelText =
      particpant.role === 'helper'
        ? 'Helfer:in'
        : `${getLevelWithNone(particpant.level).plural} ${
            particpant.role === 'leader' ? '(Leiter:in)' : ''
          }`;

    const contacts: string[] = [];
    if (
      particpant.legalGuardian?.name ||
      particpant.legalGuardian?.phoneNumber
    ) {
      contacts.push(
        `Erziehungsberechtigte:r ${particpant.legalGuardian.name} ${particpant.legalGuardian.phoneNumber},`
      );
    }
    const contactList = contacts.concat(
      particpant.contacts?.map(
        ({name, phoneNumber}) => `${name} ${phoneNumber}`
      ) || ''
    );
    // const file = await fs.promises.readFile(`documents/registrations/Anmeldebogen-131213.pdf`)
    const file = await fs.promises.readFile(
      `documents/registrations/Anmeldung-${particpant.tribe}.pdf`
    );
    const pdfDoc = await PDFDocument.load(file);
    const form = pdfDoc.getForm();
    // Get & set fields
    ...
    birthDateField.setText(
      moment(particpant.birthDate).tz('Europe/Berlin').format('DD.MM.YYYY')
    );
    ...
    insuranceField.select(
      particpant.healthInsurance === 'gkv' ? 'gesetzlich' : 'privat'
    );
    ...
    covidField.setText(
      getCovidVaccinationState(particpant.vaccinations.covid).name
    );
...
const generateLeaderInfo = async (
  participant: TeilnehmerIn
): Promise<Uint8Array | null> => {
  try {
    const file = await fs.promises.readFile(
      'documents/Leitenden-Teilnahmebedingungen.pdf'
    );
    const pdfDoc = await PDFDocument.load(file);
    const form = pdfDoc.getForm();
    // Get & set fields
    ...
    courseField.setText(
      participant.course
        ? moment(participant.course).tz('Europe/Berlin').format('DD.MM.YYYY')
        : ''
    );
    form.flatten();
    return await pdfDoc.save();
...
const sendRegistrationMail: AfterChangeHook<TeilnehmerIn> = async ({
  doc,
  req,
  operation,
}) => {
  ...
  const photoPermission = await generatePhotoPermission(
    `${doc.firstName} ${doc.lastName}`,
    moment(doc.birthDate)
  );
