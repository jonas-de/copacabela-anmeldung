...
const StrandkorbPayment: React.FC<{
  ...
  const [amounts, setAmounts] = useState(Array(strandkorbItems.length).fill(0));
  ...
  const pay = async () => {
    ...
    if (res.ok) {
      ...
      message.success(
        `${(credit - body.doc.amount).toFixed(2)} P. Restguthaben`
      );
    } else {
      setLoading(false);
      message.error(
      ...
      <Container className="pt-4 pb-4">
        <ImageHead image={tribe.image} text={firstName}>
          <>
            ...
            {role !== 'helper' && (
              <Tag style={{marginTop: 16, marginLeft: 8}} color={level.color}>
                {level.singular}
              </Tag>
            )}
            {role === 'helper' && (
              <Tag style={{marginTop: 16}} color="gray">
                Helfer:in
              </Tag>
            )}
              ...
              renderItem={(item, index) => (
                <List.Item>
                  <Row style={{width: '100%'}}>
                    ...
                    <Col xs="auto" className="ps-2 pe-4 me-auto">
        ...
        <Row className="pt-2">
          <Button type="dashed" onClick={back} style={{width: 200}}>
            Abbrechen
          </Button>
        </Row>
      </Container>
