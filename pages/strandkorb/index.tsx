...
import { AccessLevelText, getAccessLevelForHeader, getLevel } from '../../utilitites/Levels';
...
const getServerSideProps = withUser( async (context: GetServerSideUserPropsContext) => {
  ...
  if ((context.req.user.level !== "all" && context.req.user.level !== "strandkorb") || context.req.user.tribe !== "1312") {
    return {
      notFound: true
    }
  }
...
const Strandkorb: React.FC<{ accessLevel: AccessLevelText}> = ({ accessLevel}) => {
  ...
  const searchParticipant = async (id: string) => {
    ...
    const res = await defaultFetch(`/api/strandkorb/search?orderId=${id}`, "GET", undefined)
    setLoading(false)
    if (res.status === 404) {
      setNotFound(true)
    ...
    <Page level={accessLevel} showLogin={true}>
      <Container className="pt-4 pb-4">
        <Row style={{ paddingTop: 16, paddingBottom: 16 }} justify="center">
          <Col xs="auto"><h1><strong>Strandkorb</strong></h1></Col>
        </Row>
        ...
        <Row style={{ paddingTop: 4 }} justify="center">
          <Col xs="auto">
            <Checkbox style={{ color: "gray" }} checked={autoSearch} onClick={updateAutoSearch}>
              Automatisch suchen
            </Checkbox>
          </Col>
        </Row>
