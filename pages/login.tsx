...
const LoginPage: React.FC<{redirect: string}> = ({redirect}) => {
  ...
  const login = async (values: {username?: string; password?: string}) => {
    if (values.username === undefined || values.password === undefined) {
      ...
      const res = await defaultFetch(
        'api/participantscontroller/login',
        'POST',
        {
  ...
  const resetPw = async (values: {username?: string}) => {
    if (values.username === undefined) {
      await message.error('Gib deine E-Mailadresse an');
      return;
    }
    const res = await defaultFetch(
      '/api/participantscontroller/forgot-password',
      'POST',
      {
        email: values.username,
      }
    ...
    if (res.ok) {
      message
        .info('Eine E-Mail zum Zurücksetzen deines Passworts wurde verschickt')
        .then();
      setState('login');
    } else {
  ...
  const LoginForm: React.FC = () => (
    ...
    >
      <Form.Item
        label="E-Mail"
        name="username"
        rules={[{type: 'email', message: 'Gib eine gültige E-Mailadresse an'}]}
      >
        <Input type="email" style={{width: 240}} />
      </Form.Item>
