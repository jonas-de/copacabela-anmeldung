...
const getServerSideProps = withUser( async (context: GetServerSideUserPropsContext) => {
  ...
  const stats: AllStats = Object.fromEntries(TribesWithDistrict.map(tribe => [String(tribe.number), {
        name: tribe.name,
        woelflinge: 0,
        jupfis: 0,
        pfadis: 0,
  ...
  return (
    <Page showLogin={true} loggedIn={true}>
      <Container fluid="md" className="ps-0 pe-0">
        ...
        <h4>Essverhalten</h4>
        <ul>
          <li><strong>Vegan:</strong> {stats.total.vegan}</li>
          <li><strong>Vegetarisch:</strong> {stats.total.vegetarian}</li>
          <li><strong>Fleisch:</strong> {stats.total.meat}</li>
        </ul>
      </Container>
    </Page>
  )
