    ...
    const accumulatedStats = participants.docs
      ...
      .reduce((stats, tn) => {
        stats.behaviour[tn.food.eatingBehaviour][
          tn.role !== 'participant' || tn.level === 'none' ? 'leader' : tn.level
        ] += 1;
        stats.behaviour[tn.food.eatingBehaviour].total += 1;
        stats.count += 1;

        if (tn.food.intolerances) {
          stats.intolerances.push(
            `${
              tn.role !== 'participant' || tn.level === 'none'
                ? 'Leiter:in'
                : getLevel(tn.level).singular
            }: ${tn.food.intolerances}`
          );
        }
...
const Statistics: React.FC<{stats: Stats; accessLevel: AccessLevelText}> = ({
  stats,
  accessLevel,
}) => {
  return (
