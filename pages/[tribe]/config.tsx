...
import {canCreate} from '../../collections/ParticipantsController';
...
const Config: React.FC<{
  tribe: Tribe;
  controller: TeilnehmendenverwalterIn[];
  canCreate: boolean;
  accessLevel: AccessLevelText;
}> = ({tribe, controller, canCreate, accessLevel}) => {
  const router = useRouter();
  const [showNew, setShowNew] = useState(false);
  const submit = async (values: TeilnehmendenverwalterIn) => {
    const res = await defaultFetch(
      '/api/participantscontroller',
      'POST',
      values
    );
    if (res.ok) {
      ...
      <Container fluid="md" className="ps-0 pe-0">
        ...
        >
          <Table.Column
            ...
            key="name"
            sorter={(
              a: TeilnehmendenverwalterIn,
              b: TeilnehmendenverwalterIn
          ...
          {tribe.number === 1312 && (
            <Table.Column
              ...
              render={num => (
                <Link href={`/${num}/config`}>
                  {getTribeForNumber(Number(num)).name}
                </Link>
              )}
              onFilter={(value, record: TeilnehmendenverwalterIn) =>
                record.tribe === String(value)
              }
            />
          )}
          <Table.Column
